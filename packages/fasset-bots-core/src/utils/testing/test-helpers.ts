...
import { filterEvents, TruffleExtractEvent } from "../events/truffle";
import { Web3EventDecoder } from "../events/Web3EventDecoder";
import { BNish, DAYS, HOURS, MINUTES, toBN, toBNExp, toHex, WEEKS, YEARS } from "../helpers";
...
export namespace time {
     ...
     */
    export async function advanceBlockTo(target: BNish): Promise<void> {
        await mineUpTo(toHex(target));
    }
    ...
    export async function latest(): Promise<BN> {
        return toBN(await timeHH.latest());
    }
    ...
    export async function latestBlock(): Promise<BN> {
        return toBN(await timeHH.latestBlock());
    }
     ...
     */
    export async function increase(duration: BNish): Promise<void> {
        await timeHH.increase(toHex(duration));
    }
     ...
     */
    export async function increaseTo(target: BNish): Promise<void> {
        await timeHH.increaseTo(toHex(target));
    }
 ...
 */
export function ether(value: BNish): BN {
...
async function eventsForTransaction(emitter: Truffle.ContractInstance, receiptTx: string, name: string) {
    const decoder = new Web3EventDecoder(emitter.abi);
    const receipt = await web3.eth.getTransactionReceipt(receiptTx);
    const events = decoder.decodeEvents(receipt.logs);
